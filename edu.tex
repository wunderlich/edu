...
linksbündig (\texttt{c}), rechtsbündig (\texttt{r}) oder zentriert (\texttt{c}) innerhalb der jeweiligen Spalte angezeigt wird. Außerdem kann optional die vertikale Ausrichtung durch \texttt{t} (oben, Standard) und \texttt{c} (mittig) bestimmt werden.
  ...
  \renewcommand{\arraystretch}{1.2}
	\group_begin:
	  ...
	  \tl_if_eq:VnT \g_edu_exampointstabletype_tl {X} {
	    \begin{tabularx}{\linewidth}{|l|*{\clist_count:n {#1}}{>{\centering}X|}c|}
	      \hline
...
\begin{MacroCode}{class}
\NewDocumentCommand \makeexamrestable { s m m m } {
  
  \setlength{\fboxsep}{10pt}
	\fbox{
	  \parbox{\linewidth-2\fboxsep-2\fboxrule}{
		  \begin{minipage}{\g_edu_exampointstablewidth_tl-\fboxsep}
		    \exampointstable{#2}{#3}{#4}
		  \end{minipage}
		  \hfill
		  \begin{minipage}{\linewidth-\g_edu_exampointstablewidth_tl-\fboxsep}
			  \Large\sffamily\bfseries \g_edu_exampointsmarklabel_tl :~\tikz[baseline=0.75ex] \draw (0,0) rectangle (1.5,0.75);
			  \hspace{0.5cm} \LARGE \g_edu_exampointsaveragelabel_tl
