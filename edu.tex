...
\definecolor{coderule}{RGB}{145,0,0}  % used by fancyvrb and \pdfinput
...
\DeclareDocumentCommand \pdfinput { m } {
  \par
  \IfFileExists{#1.pdf}{
	  \noindent
	  \begin{minipage}{\linewidth}
		  ...
		  \par
		  \vspace*{-2.5\fboxrule}
		  \noindent\fcolorbox{coderule}{white}{
	  	  \includegraphics[width=\linewidth-2\fboxsep]{#1.pdf}
		  }
	  \end{minipage}
		\vspace{\baselineskip}
  }{
    \Warning{Die~Graphik~\texttt{#1.pdf}~ist~nicht~vorhanden.~Erzeuge~die~ Graphiken~der~Dokumentation~durch~\texttt{make~all}.}
  }
}



\DeclareDocumentEnvironment {codefilecontent} { s m } {
  \VerbatimOut{#2.tex}
} {
  \endVerbatimOut
  \codeinput{#2}
...
%
\localeDE{
Viele Einstellungen von \thepackage\ lassen sich direkt beim Laden der Dokumentenklasse konfigurieren. Dies geschieht über sogenannte \emph{Optionen}. So kann man z.\,B. die Standardschriftgröße durch die Option \opt*{fontsize} auf \opt*{12pt} erhöhen:
}
...
\endcode
%
Möchte man mehrere Optionen verwenden, werde diese jeweils durch ein Komma getrennt. Man sollte sie -- zugunsten der Lesbarkeit -- in einzelne Zeilen schreiben:
...
%%  \nopagebreak[4]
%%  \setlength{\fboxsep}{1.5pt}
%%  \noindent\rotatebox{0}{\colorbox{section}{\textcolor{white}{\scriptsize\texttt{\TeX}}}}
%\endcode

  
%{
%  \setlength{\fboxsep}{1.5pt}
%  \hfill\rotatebox{0}{\colorbox{section}{\textcolor{white}{\scriptsize\texttt{PDF}}}}\hspace*{-0.5em}\hspace*{-1.5\fboxrule}
%  
%  \vspace*{-2.5\fboxrule}
%  \noindent\fcolorbox{section}{white}{
%    \includegraphics[width=\linewidth-2\fboxsep]{doc/tut-de-exe.pdf}
%  }
%}
%\vspace{\baselineskip}
...
\quest{Dritte Frage: 5 Zeilen mit jeweils 0,75 cm Abstand über halbe Textbreite}
...
\startcode{de}
\begin{Verbatim}
\quest{Erste Frage: Multiple Choice mit leeren Kästchen}
...
\quest{Zweite Frage: Multiple Choice mit leeren Kästchen (erhöhter Abstand)}
...
\quest{Dritte Frage: Multiple Choice mit alphabetisch nummerierten Kästchen}
...
\startcode{de}
\begin{Verbatim}
\quest{Erste Frage: Multiple Choice mit leeren Kästchen in 4 Spalten}
...
\quest{Zweite Frage: Multiple Choice mit alph. nummerierten Kästchen in 3 Spalten}
...
\quest{Dritte Frage: Multiple Choice (Mathemodus & erhöhter Zeilenabstand)}
...
\startcode{de}
\begin{Verbatim}
\sol{Lösung mit Unterlösungen}

\subsol{Erste Unterlösung}
Lorem ipsum dolor sit amet ...

\subsol{Zweite Unterlösung}
Lorem ipsum dolor sit amet ...
\end{Verbatim}
\endcode
%
...
%
\localeDE{
Es kann hilfreich sein, auf erwartete Ergebnisse durch eine Linie oder eine Box hinzuweisen. Zu diesem Zweck können die Makros \Macro\resr\ (engl. \emph{rule} -- \emph{Linie}) und \Macro\resb\ (engl. \emph{box} -- \emph{Box}) verwendet werden. Beide verhalten sich grundsätzlich wie \cs{res}, zeigen die Ergebnisse also nur an, wenn \opt{showresults} gesetzt ist. Werden die Ergebnisse nicht angezeigt, erzeugt ersteres jedoch eine vertikale Linie, letztes einen rechteckigen Rahmen.

Die Linie von \Macro\resr ist standardmäßig \SI{2}{cm} lang und befindet sich \SI{4}{pt} unterhalb der Grundlinie der Zeile. Durch zwei optionale Argumente können diese Eigenschaften manipuliert werde. Lautet das Ergebnis z.\,B. "`Sonett"' und soll sich eine \SI{5}{cm} auf der Grundlinie befinden (also um \SI{0}{cm} verschoben sein), kann dies wie folgt realisiert werden:
}
...
\startcode{de}
\begin{Verbatim}
\notet{2. Aufgabe: Aufgabenstellung nicht eindeutlich verständlich.}
\end{Verbatim}
\endcode
%
\localeDE{
...
\startcode{de}
\begin{Verbatim}
\makeexamtitle[Viel Glück!][image-example.jpg][3cm]
\end{Verbatim}
\endcode
%
\localeDE{
...
\begin{Verbatim}
\begin{multiexelist}
  \item Erste Teilaufgabe \point{1}
...
\startcode{de}
\begin{Verbatim}
\exe[5]{Verständnisfragen mit insgesamt 5 VP}
...
\startcode{de}
\begin{Verbatim}
\makeexamrestable[Horst Schlämmer]{1, 2, 3, 4, 5, 6, 7}{2, 4, 6, 8, 10, 12, 14}{56}
\end{Verbatim}
\endcode
%
\localeDE{
...
\Warning{\opt*{thmbox} erfordert zwingend nummerierte Theoreme. Deshalb muss in diesem Fall zusätzlich \opt{thmimpnumbered} und \opt{thmunimpnumbered} gewählt werden.}
}
\medskip

\Warning{Irgendein Text}
...
\keys_define:nn {edu} {
  ...
  exampointstablewidth .initial:n = 0.65\linewidth,
}
...
\begin{MacroCode}{class}
\keys_define:nn {edu} {
  linepname .tl_gset:N = \g_edu_linepname_tl,    % Line in parametrical form with vector variable: Name
  linepname .initial:n = g,
  linepsupport .tl_gset:N = \g_edu_linepsupport_tl,    % Line in parametrical form with vector variable: Support vector
  linepsupport .initial:n = p,
  linepdir .tl_gset:N = \g_edu_linepdir_tl,    % Line in parametrical form with vector variable: Directional vector
  linepdir .initial:n = u,
  lineppar .tl_gset:N = \g_edu_lineppar_tl,    % Line in parametrical form with vector variable: Parameter 1
...
\begin{MacroCode}{class}
\keys_define:nn {edu} {
  planepname .tl_gset:N = \g_edu_planepname_tl,    % Plane in parametrical form with vector variable: Name
  planepname .initial:n = E,
  planepsupport .tl_gset:N = \g_edu_planepsupport_tl,    % Plane in parametrical form with vector variable: Support vector
  planepsupport .initial:n = p,
  planepdiri .tl_gset:N = \g_edu_planepdiri_tl,    % Plane in parametrical form with vector variable: Directional vector 1
  planepdiri .initial:n = u,
  planepdirii .tl_gset:N = \g_edu_planepdirii_tl,    % Plane in parametrical form with vector variable: Directional vector 2
  planepdirii .initial:n = v,
  planeppari .tl_gset:N = \g_edu_planeppari_tl,    % Plane in parametrical form with vector variable: Parameter 1
  planeppari .initial:n = r,
  planepparii .tl_gset:N = \g_edu_planepparii_tl,    % Plane in parametrical form with vector variable: Parameter 2
...
\begin{MacroCode}{class}
\keys_define:nn {edu} {
  planenvarname .tl_gset:N = \g_edu_planenvarname_tl,    % Plane in normal form with vector variable: Name
  planenvarname .initial:n = E,
  planenvarsupport .tl_gset:N = \g_edu_planenvarsupport_tl,    % Plane in normal form with vector variable: Support vector
  planenvarsupport .initial:n = p,
  planenvarnormal .tl_gset:N = \g_edu_planenvarnormal_tl,    % Plane in normal form with vector variable: Normal vector
...
This command typesets a timestamp in the format YYYYMMDDHHMMSS.
\begin{MacroCode}{class}
...
\begin{MacroCode}{class}
  ...
  \bool_if:NT \g_edu_pagecount_bool {
    \rofoot[]{\l__edu_footertextr_tl\small\normalfont\sffamily%
      \pagemark/\pageref*{LastPage}}         % gerade rechts
    \lefoot[]{\l__edu_footertextr_tl\small\normalfont\sffamily%
...
\begin{macro}{\exe*}[2]{[<number points>]}{<title>}
\begin{MacroCode}{class}
\DeclareDocumentCommand \exe { s o m }{
	...
	\fbox{
	  \parbox{\linewidth-2\fboxsep-2\fboxrule}{
	    ...
	    \par
		  ...
		  \end{minipage}
		  \hfill
		  \begin{minipage}{\linewidth-\g_edu_exampointstablewidth_tl-\fboxsep}
...
Nun werden die Theoremstyles definiert. For this purpose, two control sequences \cs{__edu_theoremstyleimp:n} and \cs{__edu_theoremstyleimp:n} are defined, which take an argument for numbered theorems (\texttt{yes} or \texttt{no}).
...
\begin{MacroCode}{class}
\DeclareDocumentCommand \linepvar { s O{\g_edu_linepname_tl} O{\g_edu_linepsupport_tl} O{\g_edu_linepdir_tl} O{\g_edu_lineppar_tl} } {
  \tl_if_empty:nF {#2} {
    #2:
  }
  \ensuremath
...
\DeclareDocumentCommand \linepvec { s O{\g_edu_linepname_tl} O{\g_edu_linepsupport_tl} O{\g_edu_linepdir_tl} O{\g_edu_lineppar_tl} } {
  \tl_if_empty:nF {#1} {
    #2:
  }
  \ensuremath
...
\begin{MacroCode}{class}
\DeclareDocumentCommand \planepvar { s O{\g_edu_planepname_tl} O{\g_edu_planepsupport_tl} O{\g_edu_planepdiri_tl} O{\g_edu_planepdirii_tl} O{\g_edu_planeppari_tl} O{\g_edu_planepparii_tl} } {
  \tl_if_empty:nF {#2} {
    #2:
  }
  \ensuremath
...
\DeclareDocumentCommand \planepvec { s O{\g_edu_planepname_tl} O{\g_edu_planepsupport_tl} O{\g_edu_planepdiri_tl} O{\g_edu_planepdirii_tl} O{\g_edu_planeppari_tl} O{\g_edu_planepparii_tl} } {
  \tl_if_empty:nF {#1} {
    #2:
  }
  \ensuremath
...
\begin{MacroCode}{class}
\DeclareDocumentCommand \planenvar { s O{\g_edu_planenvarname_tl} O{\g_edu_planenvarsupport_tl} O{\g_edu_planenvarnormal_tl} } {
  \tl_if_empty:nF {#2} {
    #2:
  }
  \ensuremath
...
\DeclareDocumentCommand \planenvec { s O{\g_edu_planenvarname_tl} O{\g_edu_planenvarsupport_tl} O{\g_edu_planenvarnormal_tl} } {
  \tl_if_empty:nF {#1} {
    #2:
  }
  \ensuremath
...
\DeclareExpandableDocumentCommand \planfigurAngles { m } {
  \tl_if_in:nnT {#1} {a} {
    \tl_put_right:Nn \__edu_planfigur_temp {\draw ($(a) + (0 \c_colon_tl 0.7)$) arc (0 \c_colon_tl 40 \c_colon_tl 0.7) ($(a) + (20 \c_colon_tl 0.5)$) node {$\alpha$};}
  }
  \tl_if_in:nnT {#1} {b} {
    \tl_put_right:Nn \__edu_planfigur_temp {\draw ($(b) + (120 \c_colon_tl 0.7)$) arc (120 \c_colon_tl 180 \c_colon_tl 0.7) ($(b) + (150 \c_colon_tl 0.45)$) node {$\beta$};}
  }
  \tl_if_in:nnT {#1} {c} {
    \tl_put_right:Nn \__edu_planfigur_temp {\draw ($(c) + (220 \c_colon_tl 0.7)$) arc (220 \c_colon_tl 300 \c_colon_tl 0.7) ($(c) + (263 \c_colon_tl 0.4)$) node {$\gamma$};}
  }
