...
\localeDE{
\Option{colortheme}\WithValues{\meta{Themename}}\AndDefault{}
...
\localeDE{
\Option{multiexenumberleft}\WithValues{\meta{Begrenzer}}\AndDefault{}
\Option{multiexenumberright}\WithValues{\meta{Begrenzer}}\AndDefault{)}
...
Da die Zuweisung initialer Werte in relativen Einheiten (em, ex etc.) bei \cs*{key}-Option durch \cs*{.initial} nicht funktioniert, werden diese mithilfe dieses Hilfsmakros am Dokumentenanfang zugewiesen.
\begin{MacroCode}{class}
...
\subsubsection{Load other packages}

\begin{option}{imakeidx}
\begin{MacroCode}{class}
\keys_define:nn {edu} {
  imakeidx .bool_gset:N = \g_edu_imakeidx_bool,           % use imakeidx package
  imakeidx .initial:n = false,
}
...
\begin{option}{sfdefaultnotoall}

Shorthand for setting the whole document in sans serif Noto font (also math)

\begin{MacroCode}{class}

\keys_define:nn {edu} {
  sfdefaultnotoall .bool_gset:N = \g_edu_sfdefaultnotoall_bool, % alles (auch Mathematik) in serifenlosem Noto
  sfdefaultnotoall .initial:n = false,
}
...
\subsubsection{Euro-Symbol}

\begin{option}{eurosymbol}
Replace character € with official Euro symbol of package \pkg{eurosym}:
\begin{MacroCode}{class}
\keys_define:nn {edu} {  
  eurosymbol .bool_gset:N = \g_edu_eurosymbol_bool,
  eurosymbol .initial:n = false,
}
...
\subsubsection{Geschlechtergerechte Sprache}

\begin{option}{gendersymbol}
\begin{option}{genderraise}
Gender-neutral language options:
\begin{MacroCode}{class}
\keys_define:nn {edu} {
  gendersymbol .tl_gset:N = \g_edu_gendersymbol_tl,
  gendersymbol .initial:n = \textasteriskcentered,
  genderraise .dim_gset:N = \g_edu_genderraise_dim,
}

\__edu_keys_initial:n {genderraise = 0ex}
...
\begin{option}{resultgridheight}
\begin{option}{resultgridfg}
\begin{option}{resultrulevoffset}
...
\keys_define:nn {edu} {
  ...
  resultgridheight .dim_gset:N = \g_edu_resultgridheight_dim,
  resultgridheight .initial:n = 3cm,
  resultgridfg .tl_gset:N = \g_edu_resultgridfg_tl,
  resultgridfg .initial:n = wuDarkGray,
...
% Hack, because xsim also defines a solution environment. Can be removed,
% when xsimverb can be used standalone again.
\let\solution\relax
...
\subsubsection{Index/Stichwortverzeichnis, Bibliographie und Hyperref}

\begin{MacroCode}{class}
\bool_if:NT \g_edu_imakeidx_bool {
...
Möchte man das komplette Dokument (inkl. Formelsatz durch \pkg{edumath}) in serifenlosem Noto-Font setzen, kann man diese Option wählen. Auch als Serifen- und Monospace-Schrift wird Noto geladen.

\begin{MacroCode}{class}
\bool_if:NT \g_edu_sfdefaultnotoall_bool {
  \keys_set:nn {edu} {
    sfdefault = true,
    rmfont = noto, sffont = noto, ttfont = noto,
    edumath = true
  }
  \tl_gput_right:Nn \g_edu_edumathoptions_tl {, rmmathfont=noto, sfmath}
}
...
Über die Optionen \opt{rmfont}, \opt{sffont} und \opt{ttfont} kann jeweils die Standardschrift für Serifenschrift, serifenlose Schrift bzw. nichtproportionale Schrift gewählt werden:
...
\bool_if:NF \g_edu_sfdefault_bool {            % Serif fonts only if not sfdefault
  ...
  \tl_if_eq:VnT \g_edu_rmfont_tl {noto} {
    \RequirePackage[scaled=0.85]{noto-serif}
    \tl_gput_right:Nn \g_edu_edumathoptions_tl {, rmmathfont=noto}
  }
...
\tl_if_eq:VnT \g_edu_sffont_tl {noto} {
  \RequirePackage[scaled=0.85]{noto-sans}
}
...
\tl_if_eq:VnT \g_edu_ttfont_tl {noto} {
  \RequirePackage[scaled=0.75]{noto-mono}
}
...
\begin{macro}{\tseries}
\begin{macro}{\elseries}
\begin{macro}{\lseries}
\begin{macro}{\mseries}
\begin{macro}{\sbseries}
\begin{macro}{\bseries}
\begin{macro}{\ebseries}
\begin{macro}{\blseries}

Commands for setting the font weight to thin (\cs{tseries}), extra light (\cs{elseries}), light (\cs{lseries}), medium (\cs{mseries}), semi bold (\cs{sbseries}), bold (\cs{bseries}), extra bold (\cs{ebseries}) or black (\cs{blseries}) (if avaiable).

\begin{MacroCode}{class}
\DeclareRobustCommand{\tseries}{\fontseries{ul}\selectfont}
...
\DeclareRobustCommand{\blseries}{\fontseries{ub}\selectfont}
...
\begin{macro}{\textt}[1]{<text>}
\begin{macro}{\textel}[1]{<text>}
\begin{macro}{\textl}[1]{<text>}
\begin{macro}{\textm}[1]{<text>}
\begin{macro}{\textsb}[1]{<text>}
\begin{macro}{\textb}[1]{<text>}
\begin{macro}{\texteb}[1]{<text>}
\begin{macro}{\textbl}[1]{<text>}

Commands for setting the text in the font weight thin (\cs{textt}), extra light (\cs{textel}), light (\cs{textl}), medium (\cs{textm}), semi bold (\cs{textsb}), bold (\cs{textb}), extra bold (\cs{texteb}) or black (\cs{textbl}) (if avaiable).
...
\DeclareTextFontCommand{\textbl}{\blseries}
...
Falls durch die Option \opt{eurosymbol} gewünscht, wird das Eurozeichen der Tastatur als \cs{euro}-Makro auffassen, das mithilfe des Packages \textsf{eurosym} das offizielle Eurosymbol setzt:
\begin{MacroCode}{class}
\bool_if:NT \g_edu_eurosymbol_bool {
...
Geschlechtergerechte Sprache kann mithilfe dieses Makros und dem zugehörigen Symbol umgesetzt werden. Standardmäßig wird hierfür ein sog.~Gendersternchen durch \cs{textasteriskcentered} verwendet. Optional kann die Höhe des Symbols angepasst werden, was – je nach Schriftart – sinnvoll sein kann.
\begin{MacroCode}{class}
\DeclareDocumentCommand \gend { O{\g_edu_genderraise_dim} } {
  \raisebox{#1}{\g_edu_gendersymbol_tl}
}

\end{MacroCode}


\subsubsection{Tabellen-Linien}
...
\DeclareDocumentCommand \exe { s o m o }{
  ...
  \setlength{\parfillskip}{0em} % Damit Punktzahl bei parskip wirklich rechtsbündig ist. Sonst rechts Abstand in letzter (einziger) Zeile.
...
\begin{macro}{\resrfill}[4]{[<number lines>]}{[<lineskip>]}{<text>}{[<vertical offset>]}
...
Durch die optionalen Argumente können auch mehrere horizontale Linien erzeugt und deren Abstand spezifiziert werden.
\begin{MacroCode}{class}
\DeclareDocumentCommand \resrfill { O{1} O{0.75cm} +m !O{\g_edu_resultrulevoffset_dim} } {%
  \bool_if:NTF \g_edu_showresults_bool {%
    \group_begin:
      \color{\g_edu_resultfg_tl} #3
    \group_end:
  }{%
    \leavevmode\leaders\hbox{\rule[-\g_edu_resultrule_dim+#4]{1pt}{\g_edu_resultrule_dim}}\hfill\kern0pt\hspace*{-\parfillskip}

    \prg_replicate:nn {#1 - 1} {

      \bool_if:nT { \int_compare_p:n { #1 > 1 } } {
        \par
        \vspace{-\baselineskip}
        \parskipreduce
        \vspace{#2}
...
\begin{macro}{\resg}[3]{[<grid height>]}{[<grid width>]}{<text>}
Zeigt Ergebnisse in Abhängigkeit der Option \opt{showresults} an. Ansonsten wird ein horizontale Linie beliebiger Länge angezeigt.
\begin{MacroCode}{class}
\DeclareDocumentCommand \resg { O{\g_edu_resultgridheight_dim} O{\linewidth} +m } {%
  \bool_if:NTF \g_edu_showresults_bool {%
    \group_begin:%
      \color{\g_edu_resultfg_tl}#3
    \group_end:%
  }{%
    \tikz \draw[step=0.5, color=\g_edu_resultgridfg_tl] (0,0) grid +(
      \fp_eval:n { floor(\dim_to_decimal_in_unit:nn { #2 } { 0.5cm }) * 0.5 },
      \fp_eval:n { floor(\dim_to_decimal_in_unit:nn { #1 } { 0.5cm }) * 0.5 }
    );
  }%
  \bool_if:NT \g_edu_resultresetmultiexe_bool {
    \rstmultiexe
...
%
  \parskipreduce
  \vspace{0.625\baselineskip} % Quick and dirty fix of missing space with TeXLive 2024
  \begin{adjustbox}{valign=\g__edu_colalign_tl}
  ...
  \vspace{-0.05\baselineskip} % Quick and dirty fix of missing space with TeXLive 2024
  \parskipreduce
}
...
%  \vspace{1.5\multicolsep} % Erhöht, da bei [t] minipage vorher keinen normalen baselineskip hat.
  \vspace{0.03\baselineskip} % Quick and dirty fix of missing space with TeXLive 2024
  \begin{cols2}[#1]
}{%
  \end{cols2}%
  \vspace{0.025\baselineskip} % Quick and dirty fix of missing space with TeXLive 2024
  \tl_gset:Nn \g__edu_colalign_tl {t}
}
...
%
  ...
  \parskipreduce
  \vspace{0.75\baselineskip}
  \vspace{-0.065\baselineskip} % Quick and dirty fix of missing space with TeXLive 2024
  \IfBooleanT {#1} {
    \tl_gset:Nn \g__edu_colalign_tl {t}
  }
}
...
\DeclareDocumentEnvironment {graphicscol} { s O{0.5\linewidth} m O{width=\linewidth} o O{t} } {%
  ...
  \IfBooleanF {#1} {
    \vspace{0.175\baselineskip} % Quick and dirty fix of missing space with TeXLive 2024
  }
}
...
\lstdefinestyle{lstresstyle}{%
  aboveskip=0.75\baselineskip,
  belowskip=\g_edu_lstbelowskip,
  basicstyle=\ttfamily\color{\g_edu_resultfg_tl},
