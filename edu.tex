...
\begin{filecontents}{edu.bib}
    ...
    @online{tikz,
        author = {Till Tantau},
        title = {pgf -- Create PostScript and PDF graphics in \tex},
        year = {2008},
...
\localeDE{
\Option{rmfont}\WithValues{\meta{PSNFSS-Abkürzung}}\AndDefault{cmr}
\Option{sffont}\WithValues{\meta{PSNFSS-Abkürzung}}\AndDefault{cmss}
\Option{ttfont}\WithValues{\meta{PSNFSS-Abkürzung}}\AndDefault{cmtt}
Mithilfe dieser Optionen können Schriftarten der verschiedenen Schriftfamilien des Dokuments verändert werden. Die Bedeutung der Schriftfamilien kann Tabelle~\ref{tab:fontfamilies} entnommen werden.

 \begin{table}[h!tb]
  \centering
  \caption{Schriftfamilien und zugehörige Optionen.}
  \label{tab:fontfamilies}
  \medskip
   \begin{tabular}{llll}\toprule
     \sffamily Familie & \sffamily Option & \sffamily Beispiel \\\midrule
     Serifenschrift & \opt{rmfont} & \textrm{Beispieltext} \\
     Serifenlose Schrift & \opt{sffont} & \textsf{Beispieltext} \\
     Nichtproportionale Schrift & \opt{ttfont} & \texttt{Beispieltext} \\ \bottomrule
   \end{tabular}
 \end{table}

\meta{PSNFSS} bezieht sich aus die Abkürzungen der Schriftarten des Packages \pkg*{psnfss} (\emph{PostScript standard Symbol and Dingbats fonts}). Die Dokumentation des Packages liefert einen Überblick über die Verwendung und die Namen der Schriftarten \footcite{fonts}.
  
Diese Optionen arbeiten auf einer sehr rudimentären Ebene. Komfortabler ist die Verwendung geeigneter Packages für Schriften. Einige dieser Packages können direkt über \thepackage\ geladen werden. Die hierzu nötigen Optionen werden im Folgenden beschrieben. \textbf{Diese Optionen sind der Verwendung von \opt{rmfont}, \opt{sffont} und \opt{ttfont} vorzuziehen!} Eine umfangreiche Übersicht zu weiteren Schriftarten bietet außerdem \emph{The \LaTeX\ Font Catalogue} \footcite{fontcatalogue}.
}
...
\Option{left}\WithValues{\meta{Linker Seitenrand}}\AndDefault{15mm}
...
\Option{twoupleft}\WithValues{\meta{Linker Seitenrand}}\AndDefault{20mm}
...
\Option{transparencyleft}\WithValues{\meta{Linker Seitenrand}}\AndDefault{15mm}
...
\localeDE{\thepackage\ definiert die Makros \cs{seeexercise}, \cs{seefigurelabel}, \cs{seelistinglabel}, \cs{seesectionlabel} und \cs{seesolutionlabel} für Verweise auf die entsprechenden Textelemente (Aufgaben, Lösungen, Abbildungen, Abschnitte), standardmäßig in der Form (s.\,Abb.\,X). Abbildung~\ref{fig:see} veranschaulicht die hierbei verwendeten Optionen am Beispiel eines Verweises auf eine Abbildung (deshalb \opt*{seefigurelabel}).
...
\localeDE{\subsection{Informatik}}
...
\ExplSyntaxOn
\keys_define:nn {edu} {
  rmfont .tl_gset:N = \g_edu_rmfont_tl,                  % name of the roman font
  rmfont .initial:n = cmr,
  sffont .tl_gset:N = \g_edu_sffont_tl,                  % name of the sans-serif font
  sffont .initial:n = cmss,
  ttfont .tl_gset:N = \g_edu_ttfont_tl,                  % name of the sans-serif font
  ttfont .initial:n = cmtt,
  sfdefault .bool_gset:N = \g_edu_sfdefault_bool,          % sans-serif als familydefault
  sfdefault .initial:n = false,
  beramono .bool_gset:N = \g_edu_beramono_bool,            % use Bera Mono as typewriter
  beramono .initial:n = true,
  lato .bool_gset:N = \g_edu_lato_bool,                    % use lato as sans-serif (same as sffont=fla)
  lato .initial:n = false,
  palatino .bool_gset:N = \g_edu_palatino_bool,            % use mathpazo
  palatino .initial:n = true,
  sourcecodepro .bool_gset:N = \g_edu_sourcecodepro_bool,  % use Source Code Pro
  sourcecodepro .initial:n = false,
  sourcesanspro .bool_gset:N = \g_edu_sourcesanspro_bool,  % use Source Sans Pro
  sourcesanspro .initial:n = true,
}
...
\begin{MacroCode}{class}
\keys_define:nn {edu} {
  transparencytop .dim_gset:N = \g_edu_transparencytop_dim,        % Top margin at twoup
  transparencytop .initial:n = 10mm,
  transparencyright .dim_gset:N = \g_edu_transparencyright_dim,    % Right margin at twoup
  transparencyright .initial:n = 10mm,
  transparencybottom .dim_gset:N = \g_edu_transparencybottom_dim,  % Bottom margin at twoup
  transparencybottom .initial:n = 10mm,
  transparencyleft .dim_gset:N = \g_edu_transparencyleft_dim,      % Left margin at twoup
  transparencyleft .initial:n = 15mm,
}  
...
\keys_define:nn {edu} {
  ...
  exepointsstyle .initial:n = \small\bfseries\sffamily,
}
...
\keys_define:nn {edu} {
  ...
  subexepointsstyle .initial:n = \scriptsize\bfseries\sffamily,
}
...
\keys_define:nn {edu} {
  ...
  resultrulelength .initial:n = 5cm,
}
...
\begin{MacroCode}{class}
\renewcommand{\rmdefault}{\g_edu_rmfont_tl}
\renewcommand{\sfdefault}{\g_edu_sffont_tl}
\renewcommand{\ttdefault}{\g_edu_ttfont_tl}
...
\begin{MacroCode}{class}
\bool_if:NT \g_edu_beramono_bool {
  \RequirePackage[scaled=0.85]{beramono}
}

\bool_if:nT {\g_edu_lato_bool || \str_if_eq_p:nn {\g_edu_sffont_tl}{fla}}  {
...
\bool_if:nT {\g_edu_palatino_bool && !\g_edu_sfdefault_bool} {
...
\bool_if:NT \g_edu_sourcecodepro_bool {
  \RequirePackage{sourcecodepro}
}

\bool_if:NT \g_edu_sourcesanspro_bool {
...
\DeclareDocumentCommand \exe { o m }{
  ...
  \IfNoValueTF {#1} {
    ...
    \tl_set:Nn \l__edu_exepoints_tl {
      \g_edu_exepointsleft_tl#1\g_edu_exepointslabel_tl\g_edu_exepointsright_tl
    }
  }
...
\DeclareDocumentCommand \subexe { o m }{
  ...
  \IfNoValueTF {#1} {
    \tl_clear:N \__edu_subexepoints
  }{
    \tl_set:Nn \__edu_subexepoints {
      \g_edu_subexepointsleft_tl#1\g_edu_subexepointslabel_tl\g_edu_subexepointsright_tl
    }
  }
...
\begin{MacroCode}{class}
\DeclareDocumentCommand \resr { O{\g_edu_resultrulelength_dim} m } {%
  \bool_if:NTF \g_edu_showresults_bool {
    \textcolor{\g_edu_resultfg_tl}{#2}%
  }{\raisebox{-\g_edu_resultrule_dim}{\rule{#1}{\g_edu_resultrule_dim}}}%
}
...
\DeclareDocumentEnvironment {bbpart} { s O{t} m } {
  ...
  \fbox{\usebox{\bbbox}}
  \hspace{-2\fboxrule}%
  \hspace{-1.75pt} % Removes strange space between minipages.
}
