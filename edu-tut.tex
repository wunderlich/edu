...
\localeDE{
Durch ein weiteres optionales Argument ermöglicht es \Macro\resrfill, mehrere horizontale Linien zu erzeugen. Die erste Linie füllt die aktuelle Zeile, die folgenden Linien jeweils ganze Zeilen. Der Abstand der Linien – standardmäßig \SI{0,75}{cm} – kann über ein zusätzliches optionales Argument angegeben werden.

Im folgenden Beispiel lautet das Ergebnis jeweils "`Lampukistan"', wobei zuerst 2 Linien im Abstand von \SI{0,75}{cm} (Standard) und im zweiten Fall drei Zeilen im Abstand von \SI{0,5}{cm} erzeugt werden:
}

\startcode{de}
\begin{codefilecontent}{doc/tut-de-content-resrfill-multiple-lines}
\eduoption{showresults}{false}
...
Für Rechnungen u.\,Ä. können mithilfe von \Macro\resg Raster (engl. \emph{grid} – Raster) erzeugt werden. Breite und Höhe des Rasters ist jeweils immer ein Vielfaches von \SI{0.5}{cm}, wobei abgerundet wird (z.\,B. \SI{2.9}{cm} → \SI{2.5}{cm}). Standardmäßig ist es so breit, dass es die aktuelle Zeile (\cs{linewidth}) maximal ausfüllt und \SI{3}{cm} hoch. Durch die Option \opt{resultgridheight} kann diese Standardhöhe global angepasst werden. Die graue Farbe kann durch \opt{resultgridfg} angepasst werden. Außerdem können Höhe und Breite individuell bei jedem Aufruf durch zwei optionale Argumente spezifiziert werden.
